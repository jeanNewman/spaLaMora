<template>
<div>
  <h1>Vitae autem degendae ratio maxime quidem illis placuit quieta.</h1>

<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Eadem fortitudinis ratio reperietur. Quod equidem non reprehendo; Illi enim inter se dissentiunt. At enim hic etiam dolore. </p>

<pre>Quid autem apertius quam, si selectio nulla sit ab iis
rebus, quae contra naturam sint, earum rerum, quae sint
secundum naturam, fore ut tollatur omnis ea, quae quaeratur
laudeturque, prudentia?

Hic Speusippus, hic Xenocrates, hic eius auditor Polemo,
cuius illa ipsa sessio fuit, quam videmus.
</pre>


<ol>
	<li>Putabam equidem satis, inquit, me dixisse.</li>
	<li>Piso igitur hoc modo, vir optimus tuique, ut scis, amantissimus.</li>
	<li>Quis hoc dicit?</li>
	<li>Eodem modo is enim tibi nemo dabit, quod, expetendum sit, id esse laudabile.</li>
</ol>

<div>
<p>Utilitatis causa amicitia est quaesita. Duo Reges: constructio interrete. <a href="http://loripsum.net/" target="_blank">Quod vestri non item.</a> Nunc agendum est subtilius. </p>
</div>
 <blockquote cite="http://loripsum.net">
	Ut vero conservetur omnis homini erga hominem societas, coniunctio, caritas, et emolumenta et detrimenta, quae felmata et blammata appellant, communia esse voluerunt;
</blockquote> 

</div>

</template>

<script>
export default {

}
</script>

<style>

</style>